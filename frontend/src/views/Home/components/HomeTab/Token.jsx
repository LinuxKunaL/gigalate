...
function Token() {
  ...
  useEffect(() => {
    ...
    fetchTokens();
  }, [currentAccount?.address, updateUseEffect]);
  ...
  return (
    <>
      <div className="w-full relative h-[15pc] rounded-t-mds border-[1px]z p-2">
        <div className="flex flex-col gap-2 overflow-auto h-[14.5pc]">
          {listTokens?.length > 0 ? (
...
function SendToken({ props }) {
    ...
    try {
      const tx = await tokenContract.methods
        .transfer(
          ...
          web3.utils.toWei(sendTokenData?.amount, "ether")
        )
  ...
  const calculateGas = async (_to, _from, _value) => {
    ...
    try {
      const encodedValue = web3.eth.abi
        .encodeParameter("uint256", _value)
        .slice(2);
      const encodedFrom = web3.eth.abi
        .encodeParameter("address", _from)
        .slice(2);
...
function ViewToken({ props }) {
  ...
  const currentAccount = getCurrentAccount();
  const tokenContractAddress = props?.tokenData?.contractAddress;
  ...
  const removeToken = (contractAddress) => {
    ...
    props?.setUpdateUseEffect(+1);
  };

  useEffect(() => {
    const fetchingTokenActivity = async () => {
      const response = await getActivityByAddress(
        currentAccount,
        "token",
  ...
  }, [currentAccount, tokenContractAddress]);
            ...
            activityData?.map((data, index) => (
              ...
              >
                <div className="h-9 relative bg-onyx-900 w-10 flex justify-center items-center rounded-full text-indigo-500">
                  {data.sender === currentAccount.address ? (
                    <BsSendCheckFill />
                  ) : (
                    <FaCoins />
                  )}
...
function ImportToken({ props }) {
  ...
  const displayTokenDetails = async (_contractAddress) => {
    ...
    const tokenContract = new web3.eth.Contract(tokenABI, _contractAddress);

    const tokenSymbol = await tokenContract.methods.symbol().call();
    const tokenName = await tokenContract.methods.name().call();
    const tokenDecimal = await tokenContract.methods.decimals().call();
    const forUserBalance = await tokenContract.methods
      .balanceOf(currentAccount.address)
      .call();
    ...
    setTokenDetails({
      ...
      forUserBalance: web3.utils.fromWei(Number(forUserBalance), "ether"),
    });
  };
